...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            "- 当工具已返回同业/分位数据时，不允许声称'数据源未能获取完整行业分类信息'"
            "- 不允许把技术面低位、均线折价、布林带位置写成PE/PB/PEG估值判断"
            "✅ 你必须："
            "- 立即调用统一基本面分析工具"
            "- 等待工具返回真实数据"
            "- 基于真实数据进行分析"
        ...
        system_prompt = (
            ...
            "   - 公司基本信息和财务数据分析"
            "   - PE、PB等估值指标分析；PEG仅在工具返回盈利增速或PEG时分析，否则说明数据不足"
            "   - 同行业均值/中位数和3-5只可比公司对比，必须使用工具返回的同业表"
            "   - PE/PB/收盘价历史分位分析"
            "   - 当前股价是否被低估或高估的判断"
            "   - 合理价位区间和目标价位建议"
            "   - 基于基本面的投资建议（买入/持有/卖出）"
