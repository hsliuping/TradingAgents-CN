...
class AKShareProvider(BaseStockDataProvider):
            ...
            if symbol:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        # 验证返回的数据类型
                        if news_df is None:
                            self.logger.warning(f"⚠️ {symbol} AKShare返回空数据 (尝试 {attempt + 1}/{max_retries})")
                            if attempt < max_retries - 1:
                                continue
                            return None
                        ...
                        if not isinstance(news_df, pd.DataFrame):
                            self.logger.warning(f"⚠️ {symbol} AKShare返回非DataFrame格式: {type(news_df)} (尝试 {attempt + 1}/{max_retries})")
                            if attempt < max_retries - 1:
                                continue
                            return None

                        if news_df.empty:
                            self.logger.warning(f"⚠️ {symbol} AKShare新闻DataFrame为空 (尝试 {attempt + 1}/{max_retries})")
                            if attempt < max_retries - 1:
                                continue
                            return None
                        ...
                        self.logger.error(f"❌ {symbol} AKShare新闻JSON解析错误 (尝试 {attempt + 1}/{max_retries}): {je}")
                        self.logger.warning(f"⚠️ 东方财富API可能已失效或被反爬虫机制阻止")

                        # 如果是JSON解析错误，尝试备用新闻源
                        if attempt == max_retries - 1:
                            backup_news = self._get_backup_news(symbol, limit)
                            if backup_news is not None:
                                self.logger.info(f"✅ {symbol} 备用新闻源获取成功: {len(backup_news)} 条")
                                return backup_news

                        if attempt < max_retries - 1:
                            retry_delay *= 2  # JSON错误时增加等待时间
                            continue
                        return None

                    except requests.exceptions.RequestException as re:
                        self.logger.error(f"❌ {symbol} AKShare新闻网络请求错误 (尝试 {attempt + 1}/{max_retries}): {re}")
                        if attempt < max_retries - 1:
                            retry_delay *= 2
                            continue
                        return None
                        ...
                        error_msg = str(api_e)
                        self.logger.error(f"❌ {symbol} AKShare新闻API错误 (尝试 {attempt + 1}/{max_retries}): {api_e}")

                        # 检查是否是东方财富API失效的典型错误
                        if "JSONDecodeError" in error_msg and attempt == max_retries - 1:
                            backup_news = self._get_backup_news(symbol, limit)
                            if backup_news is not None:
                                self.logger.info(f"✅ {symbol} 备用新闻源获取成功: {len(backup_news)} 条")
                                return backup_news
                ...
                try:
                    ...
                    self.logger.error(f"❌ AKShare市场新闻JSON解析错误: {je}")
                except requests.exceptions.RequestException as re:
                    self.logger.error(f"❌ AKShare市场新闻网络请求错误: {re}")
                except Exception as e:
                    self.logger.error(f"❌ AKShare市场新闻获取失败: {e}")

                return None

        except ImportError as ie:
            self.logger.error(f"❌ AKShare库导入失败: {ie}")
            return None
        except Exception as e:
            self.logger.error(f"❌ AKShare新闻获取失败: {e}", exc_info=True)
            return None
        ...
        try:
            ...
            if symbol:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        news_df = await asyncio.wait_for(
                            asyncio.to_thread(ak.stock_news_em, symbol=symbol_6),
                            timeout=30  # 30秒超时
                        )
                        ...
                        for _, row in news_df.head(limit).iterrows():
                            try:
                                ...
                                summary = str(row.get('新闻摘要', '') or row.get('摘要', '') or '').strip()
                                url = str(row.get('新闻链接', '') or row.get('链接', '') or '').strip()
                                source = str(row.get('文章来源', '') or row.get('来源', '') or '东方财富').strip()
                                author = str(row.get('作者', '') or '').strip()
                                publish_time = str(row.get('发布时间', '') or row.get('时间', '') or '').strip()
                                ...
                                news_item = {
                                    ...
                                    "source": source,
                                    "author": author,
                                    "publish_time": self._parse_news_time(publish_time),
                                ...
                                self.logger.warning(f"⚠️ 处理{symbol}新闻行数据时出错: {row_e}")
                                continue
                        ...
                        self.logger.warning(f"⚠️ {symbol} AKShare新闻获取超时 (尝试 {attempt + 1}/{max_retries})")
                        if attempt < max_retries - 1:
                            await asyncio.sleep(retry_delay)
                            continue
                        return []
                        ...
                        self.logger.error(f"❌ {symbol} AKShare新闻JSON解析错误 (尝试 {attempt + 1}/{max_retries}): {je}")
                        if attempt < max_retries - 1:
                            await asyncio.sleep(retry_delay * 2)  # JSON错误时增加等待时间
                            continue
                        return []
                        ...
                        # 检查是否是东方财富API失效的典型错误
                        if "JSONDecodeError" in error_msg and attempt == max_retries - 1:
                            backup_news_df = self._get_backup_news(symbol, limit)
                            if backup_news_df is not None:
                                self.logger.info(f"✅ {symbol} 备用新闻源获取成功: {len(backup_news_df)} 条")
                                # 将DataFrame转换为新闻列表
                                news_list = []
                                for _, row in backup_news_df.head(limit).iterrows():
                                    try:
                                        title = str(row.get('新闻标题', '') or '').strip()
                                        content = str(row.get('新闻内容', '') or '').strip()
                                        source = str(row.get('文章来源', '') or '备用新闻源').strip()
                                        ...
                                        self.logger.warning(f"⚠️ 处理{symbol}备用新闻行数据时出错: {row_e}")
                                        continue
                ...
                try:
                    # 获取财经新闻，同样增加错误处理
                    news_df = await asyncio.wait_for(
                        asyncio.to_thread(ak.news_cctv),
                        timeout=30
                    )
                    ...
                    if news_df is not None and not news_df.empty:
                        ...
                        for _, row in news_df.iterrows():
                            try:
                                ...
                                content = str(row.get('content', '') or row.get('内容', '') or '').strip()
                                summary = str(row.get('brief', '') or row.get('摘要', '') or '').strip()
                                url = str(row.get('url', '') or row.get('链接', '') or '').strip()
                                source = str(row.get('source', '') or row.get('来源', '') or 'CCTV财经').strip()
                                author = str(row.get('author', '') or '').strip()
                                publish_time = str(row.get('time', '') or row.get('时间', '') or '').strip()
                                ...
                                self.logger.warning(f"⚠️ 处理市场新闻行数据时出错: {row_e}")
                                continue
                    ...
                    self.logger.warning("⚠️ AKShare市场新闻获取超时")
                except json.JSONDecodeError as je:
                    self.logger.error(f"❌ AKShare市场新闻JSON解析错误: {je}")
                except requests.exceptions.RequestException as re:
                    self.logger.error(f"❌ AKShare市场新闻网络请求错误: {re}")
                except Exception as e:
                    self.logger.debug(f"AKShare市场新闻获取失败: {e}")
            ...
            self.logger.error(f"❌ 获取AKShare新闻失败 symbol={symbol}: {e}", exc_info=True)
            return None
    ...
    def _get_backup_news(self, symbol: str, limit: int = 10) -> Optional[pd.DataFrame]:
        """
        获取备用新闻源
        ...
        try:
            ...
            self.logger.info(f"🔄 尝试为{symbol}获取备用新闻源...")
            ...
            try:
                ...
                if market_data is not None and not market_data.empty:
                    # 查找与该股票相关的数据
                    symbol_data = market_data[market_data["代码"] == symbol.zfill(6)]

                    if not symbol_data.empty:
                        self.logger.info(f"✅ 从市场数据中找到{symbol}相关资讯: {len(symbol_data)} 条")

                        # 转换为新闻格式
                        news_data = []
                        for _, row in symbol_data.head(limit).iterrows():
                            news_data.append({
                                "新闻标题": f"{row["名称"]} - 实时行情",
                                "新闻内容": f"价格: {row["最新价"]}, 涨跌幅: {row["涨跌幅"]}%, 成交量: {row["成交量"]}, 市值: {row["总市值"]}",
                ...
                self.logger.warning(f"⚠️ 市场数据备用源失败: {market_e}")

            # 方法2: 尝试央视财经新闻
            try:
                cctv_news = ak.news_cctv()
                if cctv_news is not None and not cctv_news.empty:
                    self.logger.info(f"✅ 获取央视财经新闻成功: {len(cctv_news)} 条")

                    # 转换央视新闻为标准格式
                    news_data = []
                    for _, row in cctv_news.head(limit).iterrows():
                        news_data.append({
                            '新闻标题': str(row.get('title', '')).strip(),
                            '新闻内容': str(row.get('content', '')).strip(),
                            '发布时间': str(row.get('date', '')).strip(),
                            '文章来源': '央视财经',
                            '新闻链接': '',
                        })
                ...
                self.logger.warning(f"⚠️ 央视新闻备用源失败: {cctv_e}")

            # 方法3: 尝试新浪财经新闻
            try:
                sina_news = ak.stock_news_sina()
                if sina_news is not None and not sina_news.empty:
                    self.logger.info(f"✅ 获取新浪财经新闻成功: {len(sina_news)} 条")
                    return sina_news.head(limit)

            except Exception as sina_e:
                self.logger.warning(f"⚠️ 新浪新闻备用源失败: {sina_e}")

            # 方法4: 生成基础股票信息作为最后备选
            self.logger.warning(f"⚠️ 所有新闻源都失败，为{symbol}生成基础数据")

            # 返回基础股票信息作为"新闻"
            basic_data = {
                "新闻标题": [f"{symbol} 市场信息"],
                "新闻内容": [f"当前获取{symbol}详细新闻遇到技术问题，这是系统生成的市场信息。请稍后重试或查看其他资讯渠道。"],
                "发布时间": [datetime.now().strftime("%Y-%m-%d %H:%M:%S")],
                "文章来源": ["系统提示"],
                "新闻链接": [""],
            }
            ...
            self.logger.error(f"❌ 备用新闻源获取失败: {e}")
            return None
