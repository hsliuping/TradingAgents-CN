...
try:
    ...
    _project_root = _this_file.parents[2]  # web/utils/report_exporter.py -> web -> 项目根目录
...
class ReportExporter:
    ...
    def _handle_emoji(self, content: str) -> str:
        """根据环境变量处理 emoji：auto(默认)/strip/ascii

        - auto: 不改动（尽量通过字体回退渲染）
        - strip: 直接移除所有 emoji 字符，避免缺字报警
        - ascii: 用简单的文本替代常见 emoji
        """
        try:
            mode = os.getenv('TRADINGAGENTS_PDF_EMOJI_MODE', 'auto').lower()
        except Exception:
            mode = 'auto'

        if mode == 'strip':
            import re
            emoji_re = re.compile(r"[\U0001F000-\U0001FAFF\U00002700-\U000027BF\U00002600-\U000026FF\U0001F1E6-\U0001F1FF]", re.UNICODE)
            return emoji_re.sub('', content)
        elif mode == 'ascii':
            replacements = {
                '🎯': '[目标]', '📊': '[图表]', '📈': '[上升]', '💰': '[资金]', '💭': '[思考]', '📰': '[新闻]', '⚠️': '[警告]', '📋': '[清单]',
                '😃': ':)', '🚀': '[火箭]', '🔥': '[火]', '✅': '[OK]', '❌': '[X]'
            }
...
"""
    ...
    def generate_docx_report(self, results: Dict[str, Any]) -> bytes:
        ...
        # 可选处理 emoji（strip/ascii）以避免 PDF 引擎缺字（用于docx同样安全）
        ...
        try:
            ...
            # 使用禁用 YAML front matter 的格式
            extra_args = []
            logger.info(f"🔧 pypandoc参数: 使用 format=markdown-yaml_metadata_block 禁用YAML解析")
    ...
    def generate_pdf_report(self, results: Dict[str, Any]) -> bytes:
        ...
        # 构建候选引擎（包含 CJK 内容时对 pdflatex 降权，并去重）
        pdf_engines = []
        added = set()
        ...
        # 如果指定了首选引擎，且该引擎可用，则置于首位；若不可用则记录并跳过
        ...
        if detected['tectonic'] and 'tectonic' not in added:
            pdf_engines.append(('tectonic', '轻量级LaTeX引擎（conda 可安装）'))
            added.add('tectonic')
        # 2) 其他 LaTeX 引擎（尽量优先 xelatex/lualatex 以便更好中文/字体支持）
        ...
        # 如果一个都没检测到，提供一个清晰的尝试顺序供提示（不添加 None，避免触发未知默认 pdflatex）
        if not pdf_engines:
            pdf_engines = [
                ('tectonic', '轻量级LaTeX引擎（conda 可安装）'),
                ('weasyprint', '现代HTML转PDF引擎'),
                ('wkhtmltopdf', 'HTML转PDF引擎，推荐安装')
            ]

        # 仅当系统已检测到某个 LaTeX 引擎时，才添加“默认”以允许 pandoc 使用其默认 LaTeX 引擎
        if any(detected[k] for k in ['pdflatex','xelatex','lualatex']):
            pdf_engines.append((None, '使用pandoc默认引擎'))
        ...
        for engine_info in pdf_engines:
            ...
            try:
                ...
                # 针对中文字体在 LaTeX 系引擎（tectonic/xelatex/lualatex）下的支持
                if engine in ('tectonic', 'xelatex', 'lualatex'):
                    # 在 macOS 上优先使用系统中文字体 PingFang SC；否则尝试 Noto Sans CJK SC
                    ...
                    # 可选：通过 fontspec Fallback 添加 Emoji 覆盖（某些引擎/fontspec 组合可能不支持）
                ...
                if engine in (None, 'tectonic', 'xelatex', 'lualatex', 'pdflatex', 'weasyprint', 'wkhtmltopdf'):
                    # 使用pypandoc将markdown转换为PDF - 禁用 YAML 解析
                    pypandoc.convert_text(
                        cleaned_content,
                        'pdf',
                    ...
                    try:
                        css_text = ''
                        if 'css_file' in locals() and css_file and os.path.exists(css_file):
