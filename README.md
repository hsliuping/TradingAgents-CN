...
- **DashScope 双端点模式**: 在同一个 `dashscope` 厂家下同时支持阿里百炼兼容模式和千问 Coding Plan
...
当前版本保留了原版 `dashscope` 厂家标识，并在同一个厂家下新增了两种端点模式：

- **阿里百炼（兼容模式）**
  - 默认端点：`https://dashscope.aliyuncs.com/compatible-mode/v1`
  - 默认模型：`qwen-turbo` / `qwen-max`
- **千问 Coding Plan**
  - 端点：`https://coding.dashscope.aliyuncs.com/v1`
  - 推荐模型：`qwen3.5-plus` / `qwen3-max-2026-01-23`

这两种模式可以在系统中同时存在：

- 厂家级配置用于设置 DashScope 的默认测试端点
- 模型级配置继续通过 `api_base` 记录实际调用的端点
- 因此原版兼容模式模型和 Coding Plan 模型可以并存、分别调用

前端新增了对应入口：

- **厂家管理**：可切换 DashScope 默认端点模式
- **模型配置**：可为每个 DashScope 模型选择所属端点模式
- **配置向导**：首次配置时可直接选择兼容模式或 Coding Plan

已有数据库会在启动时执行一次幂等迁移：

- 旧安装会恢复为“兼容模式默认 + Coding Plan 可选”
- 已迁移后的用户主动选择会被保留，不会在后续重启时反复覆盖
