#!/usr/bin/env python3
"""Dry-run first; create only production market-input indexes."""
...
async def main(execute: bool) -> None:
    expected = sum(len(ALPHAGUARD_INDEX_SPECS[name]) for name in COLLECTIONS)
    print(
        f"production_data_collections={len(COLLECTIONS)} "
        f"production_data_indexes={expected}"
    )
    if not execute:
