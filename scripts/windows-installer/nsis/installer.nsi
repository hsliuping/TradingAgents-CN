...
nsExec::ExecToLog 'powershell -ExecutionPolicy Bypass -Command "$$envFile = \"$INSTDIR\.env\"; if (-not (Test-Path $$envFile) -and (Test-Path \"$INSTDIR\.env.example\")) { Copy-Item \"$INSTDIR\.env.example\" $$envFile -Force }; if (Test-Path $$envFile) { $$content = Get-Content $$envFile -Raw -Encoding UTF8; $$content = $$content -replace \"(?m)^PORT=.*\", \"PORT=$BackendPort\"; $$content = $$content -replace \"(?m)^API_PORT=.*\", \"API_PORT=$BackendPort\"; $$content = $$content -replace \"(?m)^MONGODB_PORT=.*\", \"MONGODB_PORT=$MongoPort\"; $$uriPattern = \"(?m)^((?:MONGODB_CONNECTION_STRING|MONGODB_URL|MONGO_URI|MONGODB_URI)=mongodb://)([^@\\r\\n/?]+@)?(\\[[^\\]]+\\]|[^:/\\r\\n/?]+)(?::[0-9]+)?\"; $$replacement = \"`$$1`$$2`$$3:$MongoPort\"; $$content = [regex]::Replace($$content, $$uriPattern, $$replacement); $$content = $$content -replace \"(?m)^REDIS_PORT=.*\", \"REDIS_PORT=$RedisPort\"; $$content = $$content -replace \"(?m)^NGINX_PORT=.*\", \"NGINX_PORT=$NginxPort\"; $$utf8 = New-Object System.Text.UTF8Encoding $$false; [System.IO.File]::WriteAllText($$envFile, $$content, $$utf8) }"'
