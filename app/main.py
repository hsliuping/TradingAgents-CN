...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        from app.services.alphaguard.operations_job_service import (
            process_operations_job_requests,
        )

        scheduler.add_job(
            process_operations_job_requests,
            IntervalTrigger(minutes=1, timezone=settings.TIMEZONE),
            id="alphaguard_operations_worker",
            name="AlphaGuard受控运维任务",
            replace_existing=True,
            max_instances=1,
        )
...
# Root-level layered probes are retained alongside the legacy /api/health API.
...
app.include_router(alphaguard_operations.router, prefix="/api", tags=["alphaguard-operations"])
